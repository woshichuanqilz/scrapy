...
class SouqSpider(scrapy.Spider):
    ...
    def parse_item(self, response):
        ...
        seller_rating = response.css('.stats .inline-block small::text'). extract()[0]
        seller_rating = int(filter(unicode.isdigit,seller_rating))

#test
        # print response.url
        # print response.css('.product-title h1::text').extract()[0]
#test

        # Not all deals are discounted
        print response.css('.vip-product-info .subhead::text').extract()
        if response.css('.vip-product-info .subhead::text').extract():
            ...
            discounted = True
            savings = response.css('.vip-product-info .mesage .noWrap::text').extract()[0].replace("AED", "")
        yield {
            ...
            'CurrentPrice': response.css('.price::text').extract()[0].replace(u"\xa0", ""),
            'Discounted': discounted,
            'Savings': savings,
            # 'SoldBy': response.css('.vip-product-info .stats a::text').extract()[0],
            'SoldBy': response.css('.stats a::text').extract()[0],
